...
API를 통해 받은 뉴스 데이터를 활용해 몇 십개의 데이터 중 AI에게 질문하여 오늘의 추천 뉴스를 구현했습니다. 바쁜 와중에 모든 뉴스를 볼 수 없는 경우, 중요하다고 판단되는 뉴스를 하나만 봐야 한다면 무엇을 봐야 하는가? 라는 질문에 대한 답을 내릴 수 있다고 판단하여 OpenRouter API 중 무료 모델을 활용해 하나의 추천 뉴스를 질문하여 그 중 AI가 추천하는 한 가지 뉴스를 출력하도록 하였습니다.
