...
export default function ResultsPage() {
  ...
  if (results.length === 0) {
    return <div className="min-h-screen bg-gray-100 flex items-center justify-center">결과를 불러오는 중...</div>;
  }
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-2xl mt-2 text-gray-800">총 {results.length}문제 중 {score}개를 맞혔습니다!</p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          {results.map((result, index) => {
            ...
            if (typeof result.question.wordData.example === 'object') {
              exampleSentence = result.question.wordData.example[foreignLang as keyof Example] || result.question.wordData.example.english;
            ...
            return (
              <div key={index} className={`p-4 rounded-lg border ${result.isCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'}`}>
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold text-lg text-gray-800">{koreanWord} / {foreignWord}</p>
                  </div>
                  <button 
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                   <p className="text-sm text-gray-500">입력한 답: <span className={result.isCorrect ? 'text-green-700 font-semibold' : 'text-red-700 font-semibold'}>{result.answer}</span></p>
                   {!result.isCorrect && <p className="text-sm text-gray-500">정답: <span className="font-semibold text-gray-700">{result.question.correctAnswer}</span></p>}
                   <p className="mt-2 text-sm text-gray-500 italic">예문 ({langNameMap[foreignLang]}): "{exampleSentence}"</p>
                </div>
              </div>
            )
          })}
